...
1. Create MVP for `calendar`, working calendar data service.
   ...
   * Utilize at least one external API.
   * Cover all developed logic with autotests (DB-supported).
1. Init a backend interface service `backend`.
    ...
    * Request all internal services' data via APIs in a single foreground script launch.
        * If possible, try requesting microservices in parallel.
